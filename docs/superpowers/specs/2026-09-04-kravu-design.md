...
- **Use Case** — one unit of business logic (Clean Architecture), named verb-noun:
  `DiscoverJobs`, `EnrichJob`, `ScoreJob`, `TailorResume`, `WriteCoverLetter`.
...
entrypoints/cli.py   (Typer)  →  services/pipeline.py (Orchestrator)
   builds adapters, loads Profile        runs use cases in order:
                                          DiscoverJobs → EnrichJob → ScoreJob
                                          → TailorResume → WriteCoverLetter
...
is created by `DiscoverJobs` and advances as each use case fills its columns.
...
1. **DiscoverJobs** — JobSpy across boards for the configured searches; insert new
   jobs; dedupe by URL.
2. **EnrichJob** — fetch each job's full description (httpx + parse; LLM fallback
   for unknown layouts). Record per-job errors; never crash the run.
3. **ScoreJob** — one focused LLM call per job: compact profile + this JD → fit
...
5. **WriteCoverLetter** — conditional: decide if the role needs a cover letter;
...
|---------|---------|---------|
| `python-jobspy` | 1.1.82 | DiscoverJobs |
| `litellm` | 1.99.0 | ScoreJob / TailorResume / WriteCoverLetter |
| `typer` | 0.27.2 | entrypoints/cli |
| `rich` | 15.0.0 | CLI output |
| `httpx` | 0.28.1 | EnrichJob (fetch) |
| `selectolax` | 0.4.11 | EnrichJob (HTML/CSS parse) |
| `trafilatura` | 2.2.0 | EnrichJob (content/JSON-LD extraction) |
